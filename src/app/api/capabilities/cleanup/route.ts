...
export async function POST(request: Request) {
  try {
    ...
    const updated = capability?.execution
      ? capabilityRegistry.markDeterministicLearningFailed(capability.id)
      : null;
    return NextResponse.json({ capability: updated, deletedWorkspaceId: workspaceId });
  } catch (error) {
